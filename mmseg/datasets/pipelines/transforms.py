...
class GetCellPoseMap(object):
    ...
    def _extend_centers_gpu(self, neighbors, centers, isneighbor, Ly, Lx, n_iter=200):
        ...
        dx = grads[:, 2] - grads[:, 3]
        del grads
        mu_torch = np.stack((dy.squeeze(0), dx.squeeze(0)), axis=-2)
        return mu_torch
...
class Normalize99(object):
    ...
    def __init__(self, in_chans=3, add_noise=False):
        self.mean = np.array([0] * in_chans, dtype=np.float32)
        self.std = np.array([0] * in_chans, dtype=np.float32)
        self.to_rgb = False
    ...
    def __call__(self, results):
        ...
        if "weak_img" in results:
            weak_img = results['weak_img'].astype(np.float32)
            for channel in range(weak_img.shape[2]):
                img_c = weak_img[..., channel]
                i99 = np.percentile(img_c, 99)
                i1 = np.percentile(img_c, 1)
                if i99 - i1 > 1e-3:
                    norm_img_c = (img_c - i1) / (i99 - i1)
                    weak_img[..., channel] = norm_img_c
                else:
                    weak_img[..., channel] = 0

            results['weak_img'] = weak_img
        return results


@PIPELINES.register_module()
class MyAug(object):
    def __init__(self, jitter_vale, jitter_prob, min_sigma, max_sigma, gaussian_prob):
        self.jitter_vale = jitter_vale
    ...
    def __call__(self, results):
        ...
        if random.rand() < self.jitter_prob:
            img, _, _ = transformsgpu.colorJitter(True, data=img, s=self.jitter_vale)
        if random.rand() < self.gaussian_prob:
            img, _, _ = transformsgpu.gaussian_blur(True, data=img, min_sigma=self.min_sigma, max_sigma=self.max_sigma)
...
class MyRandomRotate(object):
    ...
    def __call__(self, results):
        ...
        if rotate:
            ...
            # rotate segs
            results["gt_semantic_seg"] = mmcv.imrotate(
            ...
            results["gt_instance"] = mmcv.imrotate(
                results["gt_instance"],
                angle=degree,
                border_value=0,
                center=self.center,
...
@PIPELINES.register_module()
class KeepWeakImg(object):
    def __call__(self, results):
        img = results["img"]
        gt_semantic_seg = results["gt_semantic_seg"]
        results["weak_img"] = img.copy()
        results["weak_gt_semantic_seg"] = gt_semantic_seg.copy()
        return results


@PIPELINES.register_module()
class BioMedicalGaussianBlur(object):
    ...
    def __init__(self,
        ...
        assert isinstance(sigma_range, (list, tuple)) and len(sigma_range) == 2
