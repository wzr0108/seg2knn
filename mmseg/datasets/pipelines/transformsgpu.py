...
def colorJitter(colorJitter, data=None, target=None, s=0.1, probs=None):
    ...
    if not (data is None):
        if colorJitter and data.shape[1] == 3:
            seq = nn.Sequential(
                kornia.augmentation.ColorJitter(brightness=s, contrast=s, saturation=s / 2., hue=s / 3.))
...
def flip(flip, data=None, target=None, probs=None):
    ...
    Args:
        ...
        data:  input data to augment BxCxHxW
        target: labels to augment BxHxW
        probs: probability masks to augment BxCxHxW
...
def mix(mask, data=None, target=None, probs=None):
    ...
    if not (data is None):
        if mask.shape[0] == data.shape[0]:
            data = torch.cat([((1 - mask[(i + 1) % data.shape[0]]) * data[i] + mask[(i + 1) % data.shape[0]] * data[
                (i + 1) % data.shape[0]]).unsqueeze(0) for i in range(data.shape[0])])

    if not (target is None):
        target = torch.cat([((1 - mask[(i + 1) % data.shape[0]]) * target[i] + mask[(i + 1) % data.shape[0]] * target[
            (i + 1) % target.shape[0]]).unsqueeze(0) for i in range(target.shape[0])])

    if not (probs is None):
        probs = torch.cat([((1 - mask[(i + 1) % data.shape[0]]) * probs[i] + mask[(i + 1) % data.shape[0]] * probs[
            (i + 1) % probs.shape[0]]).unsqueeze(0) for i in range(probs.shape[0])])
...
def random_scale_crop(scale, data=None, target=None, ignore_label=255, probs=None):
    ...
    """
    if scale != 1:
        init_size_w = data.shape[3]
        init_size_h = data.shape[2]
        ...
        if probs is not None:
            probs = nn.functional.interpolate(probs.unsqueeze(1), scale_factor=scale, mode='bilinear',
                                              align_corners=True, recompute_scale_factor=True).squeeze(1)

        final_size_w = data.shape[3]
        final_size_h = data.shape[2]
        diff_h = init_size_h - final_size_h
        diff_w = init_size_w - final_size_w
        if scale < 1:  # add padding if needed
            if diff_w % 2 == 1:
                pad_w_right = diff_w // 2 + 1
            else:
                pad_w_right = diff_w // 2
            if diff_h % 2 == 1:
                pad_h_bottom = diff_h // 2 + 1
            else:
                pad_h_bottom = diff_h // 2

            pad = nn.ConstantPad2d((diff_w // 2, pad_w_right, diff_h // 2, pad_h_bottom), 0)
            ...
            pad = nn.ConstantPad2d((diff_w // 2, pad_w_right, diff_h // 2, pad_h_bottom), ignore_label)

            if target is not None:
                target = pad(target)

        else:  # crop if needed
            w = random.randint(0, data.shape[3] - init_size_w)
            h = random.randint(0, data.shape[2] - init_size_h)
