...
class _ConvNext(ConvNeXt):
    def forward(self, x, cb_feats=None, pre_tmps=None):
        ...
        if self.downsample_layers[0] is not None:
...
class CBConvNeXt(BaseModule):
    def __init__(self, dims, **kwargs):
        ...
        for i in range(2):
            cb_module = _ConvNext(dims=dims, **kwargs)
            if i == 1:
                cb_module.downsample_layers[0] = None
        ...
        for i in range(4):
            ...
            for j in range(jrange):
                if j == 0:
                    ...
                    layer = nn.Conv2d(cb_inplanes[i + j], cb_inplanes[i], kernel_size=(1, 1))
    ...
    def forward(self, x):
        ...
        for i, module in enumerate(self.cb_modules):
            if i == 0:
                feats, tmps = module(x)
                cb_feats = self._get_cb_feats(feats)
            else:
    ...
    def _get_cb_feats(self, feats):
        ...
        for i in range(4):
            ...
            jrange = 4 - i
            for j in range(jrange):
                tmp = self.cb_linears[i][j](feats[i + j])
                h, w = feats[i].shape[-2:]
                tmp = F.interpolate(tmp, size=[h, w])
                feed += tmp
