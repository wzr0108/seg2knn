...
class CellPoseDecoupleHead(BaseDecodeHead):

    def __init__(self, **kwargs):
        self.num_classes_seg = kwargs["num_classes"]
        kwargs["num_classes"] = 4  # cellpose head
        super(CellPoseDecoupleHead, self).__init__(
        ...
        # 第二个分支
        self.embed_layers_seg = copy.deepcopy(self.embed_layers)
        self.fusion_conv_seg = copy.deepcopy(self.fusion_conv)
        self.conv_seg_seg = nn.Conv2d(self.channels, self.num_classes_seg, kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        cellpose = self.fusion_conv(torch.cat(list(_c.values()), dim=1))
        cellpose = self.cls_seg(cellpose)

        # seg
        _s = {}
        for i in self.in_index:
            _s[i] = self.embed_layers_seg[str(i)](x[i])
            if _s[i].dim() == 3:
                _s[i] = _s[i].permute(0, 2, 1).contiguous().reshape(n, -1, x[i].shape[2], x[i].shape[3])
            if _s[i].size()[2:] != os_size:
                _s[i] = resize(
                    _s[i],
                    size=os_size,
                    mode='bilinear',
        ...
        seg = self.fusion_conv_seg(torch.cat(list(_s.values()), dim=1))
        seg = self.conv_seg_seg(seg)

        return torch.cat([cellpose, seg], dim=1)
    ...
    def forward_train(self,
        ...
        """
        # bs ,c, h, w
        # 前两个通道是梯度，然后两个通道是前景背景，后N个通道是常规等的语义分割(包括背景)，这里的背景实际上重复计算了
        ...
        losses = self.losses(seg_logit[:, 4:], gt_semantic_seg, seg_weight)
        ...
        if gt_vec is not None:
            ...
            # 前景和背景的gt
            gt_fore_background = gt_semantic_seg.new_full(gt_semantic_seg.shape, 1)
            gt_fore_background[gt_semantic_seg == 0] = 0
            gt_fore_background = gt_fore_background.squeeze(1)

            loss_seg = self.criterion2(seg_logit[:, 2:4], gt_fore_background)

            losses['loss_fore_background'] = loss_seg
