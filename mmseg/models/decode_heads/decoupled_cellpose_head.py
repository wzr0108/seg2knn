...
class DecoupledCellPoseHead(BaseDecodeHead):

    def __init__(self, use_bce=False, **kwargs):
        super(DecoupledCellPoseHead, self).__init__(
        ...
        # vec分支
        self.fusion_conv_vec = ConvModule(
            in_channels=self.channels * num_inputs,
            out_channels=self.channels,
            kernel_size=1,
            norm_cfg=self.norm_cfg
        )
        self.conv_seg_vec = nn.Conv2d(self.channels, 2, kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        fuse_feats = torch.cat(list(_c.values()), dim=1)
        x = self.fusion_conv(fuse_feats)
        x = self.cls_seg(x)

        x2 = self.fusion_conv_vec(fuse_feats)
        x2 = self.conv_seg_vec(x2)
    ...
    def forward_train(self,
        ...
        loss_seg = self.criterion2(seg_logit[:, 2:], gt_semantic_seg)
