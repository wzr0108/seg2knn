...
def contrast_preparations(feat,
    ...
    if use_avg_pool:
        ...
        mask = F.interpolate(mask.float(), size=feat.shape[-2:], mode='nearest').long()  # normalize the feat
    ...
    if pred is not None:
        pred = F.interpolate(pred.float().unsqueeze(1), size=(oh, ow), mode='nearest').long()
        prob = F.interpolate(prob.unsqueeze(1), size=(oh, ow), mode='bilinear')
        pred = pred.contiguous().view(-1)
        prob = prob.contiguous().view(-1)
        msk = (pred == mask).float() * (prob > 0.95).float()
        msk = msk.bool()
    else:
...
def bank_contrastive(feat,
    ...
    if index >= 0:
        ...
        assert isinstance(bank, (list, dict)) \
               and isinstance(bank[index], list), f'bank list expected for index={index}'
        feat = feat[index]
        bank = bank[index]
        if reg_weight > 0.:
            assert isinstance(mean, (list, dict)), f'mean list expected for index={index}'
