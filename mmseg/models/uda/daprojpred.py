...
@UDA.register_module()
class DAProjPred(UDADecorator):
    def __init__(self, **cfg):
        super(DAProjPred, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        # target_logits_aug = self.get_model().encode_decode(target_img_aug, target_img_metas)
        # target_logits_aug = target_logits_aug[:, -5:, ...]
        #
        # # 计算伪标签损失
        # # 伪标签权重 bs,h,w
        ...
        #
        # loss_pseudo_seg = self.unlbl_loss1(target_logits_aug, pseudo_label_aug)
        ...
        # target_loss, target_log_vars = self._parse_losses(
        #     {
        #         "loss_pseudo_seg": loss_pseudo_seg,
        #         "pseudo_weight": pixel_wise_weight.mean()
        #     }
        # )
        ...
        if self.local_iter >= self.ramp_up_iter:
            target_losses = self.get_model().forward_train(
                target_img_aug, target_img_metas, pseudo_label_aug.unsqueeze(1), mode="aux",
